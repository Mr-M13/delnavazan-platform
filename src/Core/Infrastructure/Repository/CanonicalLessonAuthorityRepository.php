...
final class CanonicalLessonAuthorityRepository {
    ...
    /** Phase 2A.2-N: termination refuses to strand an active future canonical schedule reservation. */
    public function transition(object $lesson,string $from,string $to,string $now,int $actor):void{global $wpdb;if((new \Delnavazan\Platform\Core\Application\CanonicalLessonScheduleGuard())->activeFutureExists('lesson',(int)$lesson->id,$now))throw new \InvalidArgumentException('active_future_schedule_exists');$n=$wpdb->update($this->p.'lessons',array('lifecycle_state'=>$to,'updated_at'=>$now,'updated_by'=>$actor),array('id'=>(int)$lesson->id,'record_model'=>'canonical_term_lesson_v1','lifecycle_state'=>$from));if($n!==1)throw new \RuntimeException('Canonical Lesson changed concurrently');}
