...
abstract class BaseRepository {
 ...
 public function usable(int $id): ?object { global $wpdb; return $wpdb->get_row($wpdb->prepare("SELECT * FROM {$this->table} WHERE id=%d AND archived_at IS NULL",$id)); }
 public function find(int $id): ?object { global $wpdb; return $wpdb->get_row($wpdb->prepare("SELECT * FROM {$this->table} WHERE id=%d",$id)); }
 public function insert(array $data): int { global $wpdb; if(false===$wpdb->insert($this->table,$data)) throw new \RuntimeException($wpdb->last_error); return (int)$wpdb->insert_id; }
 ...
 public function isUidCollision(\Throwable $e):bool{global $wpdb;return (int)$wpdb->last_errno===1062&&str_contains((string)$wpdb->last_error,"'uid'");}
}
