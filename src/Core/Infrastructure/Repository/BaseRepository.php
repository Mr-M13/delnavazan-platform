...
abstract class BaseRepository {
 ...
 public function archive(int $id,string $now,?int $actor):void{global $wpdb;$data=['status'=>'archived','archived_at'=>$now,'archived_by'=>$actor];$columns=$wpdb->get_col("DESCRIBE {$this->table}");if(in_array('updated_at',$columns,true))$data['updated_at']=$now;if(in_array('updated_by',$columns,true))$data['updated_by']=$actor;$r=$wpdb->update($this->table,$data,['id'=>$id,'archived_at'=>null]);if(false===$r)throw new \RuntimeException($wpdb->last_error);if($r!==1)throw new \InvalidArgumentException('Record is not archivable');}
 public function restore(int $id,string $status,string $now,?int $actor):void{global $wpdb;$data=['status'=>$status,'archived_at'=>null,'archived_by'=>null];$columns=$wpdb->get_col("DESCRIBE {$this->table}");if(in_array('updated_at',$columns,true))$data['updated_at']=$now;if(in_array('updated_by',$columns,true))$data['updated_by']=$actor;$r=$wpdb->update($this->table,$data,['id'=>$id,'status'=>'archived']);if(false===$r)throw new \RuntimeException($wpdb->last_error);if($r!==1)throw new \InvalidArgumentException('Record is not archived');}
}
