...
abstract class BaseRepository {
 ...
 public function archive(int $id,string $now,?int $actor):void{global $wpdb;$r=$wpdb->update($this->table,['status'=>'archived','archived_at'=>$now,'archived_by'=>$actor,'updated_at'=>$now,'updated_by'=>$actor],['id'=>$id,'archived_at'=>null]);if(false===$r)throw new \RuntimeException($wpdb->last_error);if($r!==1)throw new \InvalidArgumentException('Record is not archivable');}
 public function restore(int $id,string $status,string $now,?int $actor):void{global $wpdb;$r=$wpdb->update($this->table,['status'=>$status,'archived_at'=>null,'archived_by'=>null,'updated_at'=>$now,'updated_by'=>$actor],['id'=>$id,'status'=>'archived']);if(false===$r)throw new \RuntimeException($wpdb->last_error);if($r!==1)throw new \InvalidArgumentException('Record is not archived');}
}
