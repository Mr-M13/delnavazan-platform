...
final class BookingRequestRepository {
    ...
    public function instrument(int $id): ?object { global $wpdb; return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$this->prefix}instruments WHERE id=%d", $id ) ); }
    public function course(int $id): ?object { global $wpdb; return $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$this->prefix}courses WHERE id=%d", $id ) ); }
    public function createRequest(array $data): int { global $wpdb; if ( $wpdb->insert( $this->prefix . 'booking_requests', $data ) === false ) throw new \RuntimeException( 'Booking Request persistence failed' ); return (int) $wpdb->insert_id; }
    public function assignReference(int $id, string $reference): void { global $wpdb; if ( $wpdb->update( $this->prefix . 'booking_requests', array( 'reference_code' => $reference ), array( 'id' => $id, 'reference_code' => null ) ) !== 1 ) throw new \RuntimeException( 'Booking Request reference assignment failed' ); }
    public function createSnapshot(int $request, array $contact, string $now): int { global $wpdb; $data = $contact + array( 'booking_request_id' => $request, 'snapshot_sequence' => 1, 'privacy_notice_version' => '2026-09-05', 'privacy_notice_accepted' => 1, 'privacy_notice_accepted_at' => $now, 'submission_source' => 'public_rest_booking_request', 'created_at' => $now ); if ( $wpdb->insert( $this->prefix . 'booking_request_contact_snapshots', $data ) === false ) throw new \RuntimeException( 'Booking Request contact snapshot persistence failed' ); return (int) $wpdb->insert_id; }
    public function createTime(int $request, int $sequence, array $time, string $now): int { global $wpdb; $data = $time + array( 'booking_request_id' => $request, 'sequence_number' => $sequence, 'version' => 1, 'created_at' => $now ); if ( $wpdb->insert( $this->prefix . 'booking_request_requested_times', $data ) === false ) throw new \RuntimeException( 'Booking Request requested time persistence failed' ); return (int) $wpdb->insert_id; }
