...
final class CanonicalLessonPortalCapabilityOwner implements PortalCapabilityOwnerPort {
    public function binding(int $lessonId,int $scheduleVersionId,string $purpose,?int $studentId):array {
        ...
        if(!$lesson||!CanonicalLessonAuthorityValidator::valid($lesson,$events,$lessons)||!CanonicalLessonScheduleValidator::validForLesson($lessonId,$schedules,$lessons)||!$version||(int)$version->lesson_id!==$lessonId||(int)$version->applicable_slot!==1||!$version->uid) throw new \InvalidArgumentException('portal_upstream_aggregate_invalid');
