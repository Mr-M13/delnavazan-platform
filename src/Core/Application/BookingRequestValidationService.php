...
final class BookingRequestValidationService {
    ...
    public function validate(array $input, bool $public = true): array {
        if ( $public ) {
            $allowed = array( 'requested_instrument_id', 'selected_intro_course_id', 'full_name', 'email', 'mobile', 'timezone', 'communication_language', 'whatsapp_same_as_mobile', 'whatsapp_number', 'privacy_notice_accepted', 'privacy_notice_version', 'requested_times' );
        ...
        $full = Normalizer::text( $input['full_name'] ?? null, 191, true ); $email = Normalizer::email( $input['email'] ?? null ); $mobile = Normalizer::phone( $input['mobile'] ?? null ); $timezone = Normalizer::timezone( $input['timezone'] ?? null ); $language = Normalizer::one( $input['communication_language'] ?? null, array( 'fa', 'en' ), 'communication language' );
        if ( ! $email || ! $this->phone( $mobile ) || ! $timezone ) throw new \InvalidArgumentException( 'Valid email, mobile, and IANA timezone required' ); $same = filter_var( $input['whatsapp_same_as_mobile'] ?? null, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE ); if ( $same === null ) throw new \InvalidArgumentException( 'WhatsApp mobile relationship required' ); $whatsapp = $same ? $mobile : Normalizer::phone( $input['whatsapp_number'] ?? null ); if ( ! $this->phone( $whatsapp ) ) throw new \InvalidArgumentException( 'Valid WhatsApp number required' );
        ...
        return array( 'instrument_id' => $instrument, 'course_id' => $course, 'contact' => array( 'full_name' => $full, 'email' => $email, 'mobile' => $mobile, 'timezone' => $timezone, 'communication_language' => $language, 'whatsapp_same_as_mobile' => $same ? 1 : 0, 'whatsapp_number' => $whatsapp ), 'times' => $normalized );
    }
