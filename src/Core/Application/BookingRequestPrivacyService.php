...
final class BookingRequestPrivacyService {
    ...
    public function erase(int $requestId, int $actorId, string $reason): void {
        ...
        $now=gmdate('Y-m-d H:i:s'); $this->requests->begin(); try { $request=$this->intake->requestForUpdate($requestId); if(!$request || $request->student_id!==null || $request->lifecycle_status!=='submitted' || $request->resolution_state!=='unresolved')throw new \InvalidArgumentException('Booking Request cannot be erased'); $this->coordination->terminateForPrivacyErasure($requestId,$actorId,$now,static fn(string $scope):string=>hash_hmac('sha256','coordination-privacy-erasure:'.$requestId.':'.$scope.':'.Identifier::uid(),wp_salt('dzn_coordination_audit'))); $this->assents->invalidateForPrivacyErasure($requestId,$actorId,$now,static fn(string $scope):string=>hash_hmac('sha256','teacher-assent-privacy-erasure:'.$requestId.':'.$scope.':'.Identifier::uid(),wp_salt('dzn_teacher_assent_audit'))); $this->proposals->invalidateForPrivacyErasure($requestId,$actorId,$now,static fn(string $scope):string=>hash_hmac('sha256','proposal-privacy-erasure:'.$requestId.':'.$scope.':'.Identifier::uid(),wp_salt('dzn_proposal_issuance'))); $this->intake->eraseRequestPii($requestId,$actorId,$reason,BookingRequestIdempotency::tombstoneDigest($requestId,$now),$now); (new ExceptionService())->removeTrustedBookingRequestMatchAttention($requestId); $this->requests->commit(); } catch(\Throwable $e){$this->requests->rollback();throw $e;}
    }
}
