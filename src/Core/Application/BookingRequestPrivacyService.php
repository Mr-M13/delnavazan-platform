...
final class BookingRequestPrivacyService {
    ...
    public function erase(int $requestId, int $actorId, string $reason): void {
        ...
        $now=gmdate('Y-m-d H:i:s'); $this->requests->begin(); try { $request=$this->intake->requestForUpdate($requestId); if(!$request || $request->student_id!==null || $request->lifecycle_status!=='submitted' || $request->resolution_state!=='unresolved')throw new \InvalidArgumentException('Booking Request cannot be erased'); $this->intake->eraseRequestPii($requestId,$actorId,$reason,BookingRequestIdempotency::tombstoneDigest($requestId,$now),$now); $this->requests->commit(); } catch(\Throwable $e){$this->requests->rollback();throw $e;}
    }
}
