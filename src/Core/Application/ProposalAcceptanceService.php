...
final class ProposalAcceptanceService {
    ...
    public function record(string $familyUid,string $optionUid,int $versionNumber,string $prospectiveSubjectRef,string $channel,string $evidenceAt,string $key):array{
        ...
        $now=gmdate('Y-m-d H:i:s');$this->requests->begin();try{
            ...
            $id=$this->events->insert($data);$this->events->assignReference($id,Identifier::reference('PAE',$id));$event=$this->events->eventForCommandForUpdate($keyDigest);if(!$event)throw new \RuntimeException('Provisional acceptance persistence verification failed');$this->requests->commit();return $this->response($event,true,false);
