...
if(!str_contains($proposal,'acceptedArrangementForFamilyForUpdate')||!str_contains($proposalRepo,'accepted_service_arrangements'))throw new RuntimeException('Later Proposal issuance is not frozen after final acceptance');
...
foreach(array("array('a', 'o', 'p', 'e', 'c', 'pr', 'g', 'x')",'phase-2a2g.invalid','dzn_phase_2a2g_proposal_locks_held','dzn_phase_2a2g_authority_locks_held','SELECT CONNECTION_ID()','w1.locked','w2.started','release','outcome=idempotency_conflict','closed_competing','proposal_option_outcome_events')as$n)if(!str_contains($raceSetup.$raceWorker.$raceVerify.$raceRunner.$service,$n))throw new RuntimeException('Missing deterministic final-acceptance race invariant: '.$n);
