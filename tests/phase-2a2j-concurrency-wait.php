...
for ($attempt = 0; $attempt < 600; $attempt++) {
    ...
    if ($wait) { $evidence = array('w1_connection_id' => $w1, 'w2_connection_id' => $w2, 'expected_aggregate' => 'canonical Enrolment', 'observed_table' => (string) $wait->OBJECT_NAME, 'observed_index' => (string) $wait->INDEX_NAME, 'lock_type' => (string) $wait->LOCK_TYPE, 'lock_mode' => (string) $wait->LOCK_MODE, 'lock_data' => (string) $wait->LOCK_DATA); $json = wp_json_encode($evidence, JSON_UNESCAPED_SLASHES); file_put_contents($gate . '/w2.blocked', $json . "\n"); echo 'wait_attribution=' . $json . "\n"; exit(0); }
    usleep(100000);
}
