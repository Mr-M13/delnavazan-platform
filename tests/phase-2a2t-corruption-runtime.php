...
foreach(array('payment_execution_results','payment_execution_attempts','payment_execution_dispatches') as $table)dzn_tc_assert($wpdb->query("DELETE FROM {$p}{$table}")!==false,'Failed to reset '.$table);
