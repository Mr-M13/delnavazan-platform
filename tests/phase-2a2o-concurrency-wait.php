<?php
/**
 * Attribute the gated Phase-O contender's lock wait, then let the runner release worker 1.
 *
 * MySQL Performance Schema and MariaDB information_schema lock views are both probed. Where the
 ...
 */
if (getenv('DZN_PHASE_2A2O_RUNTIME_TEST') !== 'concurrency' || !defined('WP_CLI') || !WP_CLI) exit(1);
global $wpdb;
...
$noise = array($wpdb->prefix . 'dzn_canonical_lesson_delivery_outcomes', $wpdb->prefix . 'dzn_canonical_lesson_delivery_commands', $wpdb->prefix . 'dzn_lessons', $wpdb->prefix . 'dzn_canonical_lesson_lifecycle_events', $wpdb->prefix . 'dzn_enrolment_identity_roots', $wpdb->prefix . 'dzn_canonical_lesson_schedule_versions', $wpdb->prefix . 'dzn_terms');
...
$trxState = static function () use ($quiet, $two): ?array {
    foreach ($quiet('SELECT trx_id,trx_state FROM information_schema.INNODB_TRX WHERE trx_mysql_thread_id=%d LIMIT 1', array($two)) as $row) {
        if ((string) $row->trx_state !== 'LOCK WAIT') return null;
        return array('mechanism' => 'information_schema_innodb_trx', 'trx_id' => (string) $row->trx_id, 'state' => 'LOCK WAIT');
    }
    return null;
};
...
file_put_contents($gate . '/w2.blocked', wp_json_encode(array(
    ...
    'mechanism' => $evidence === null ? (($trx && !$process) ? 'privilege_required' : ($mysql ? 'not_observed' : 'unavailable')) : $mechanism,
    'instrumentation' => array('performance_schema' => $mysql, 'innodb_trx' => $trx, 'process_privilege' => $process),
