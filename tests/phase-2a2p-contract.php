...
if(!str_contains($intake,'Controlled participant identity state required')||!str_contains($intake,'Resolved Teacher identity requires the canonical Teacher ID'))throw new RuntimeException('Provider intake must require an explicit identity state and canonical participant ID');
