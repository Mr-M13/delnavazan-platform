...
foreach (['dzn_prepare_booking_request_matches', 'CAPABILITY_VERSION', 'CAPABILITY_OPTION', 'ensure_capabilities', 'has_cap(\'dzn_prepare_booking_request_matches\')', 'Teacher assent capability installation failed', 'check_admin_referer', 'dzn_prepare_booking_request_matches_', 'Current advisory assessment', 'assess_matches', 'Assessment could not be completed.', 'catch (\\Throwable)'] as $needle) if (strpos($migration . $admin, $needle) === false) throw new RuntimeException('Missing Phase 2A.2-A protected admin contract: ' . $needle);
