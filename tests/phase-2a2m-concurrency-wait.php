 ...
 *
 * MySQL 8 Performance Schema and MariaDB information_schema lock views are both probed. When no
 * instrumentation is exposed the script records `attribution=unavailable` after confirming that
 * worker 2 has genuinely not completed; outcome determinism is still enforced by the verifier.
 */
if (getenv('DZN_PHASE_2A2M_RUNTIME_TEST') !== 'concurrency' || !defined('WP_CLI') || !WP_CLI) exit(1);
global $wpdb;
...
$noise = array($wpdb->prefix . 'dzn_enrolment_identity_roots', $wpdb->prefix . 'dzn_enrolments', $wpdb->prefix . 'dzn_terms', $wpdb->prefix . 'dzn_lessons', $wpdb->prefix . 'dzn_teacher_assignments', $wpdb->prefix . 'dzn_canonical_lesson_lifecycle_events');
...
$evidence = null; $mechanism = 'unavailable';
$started = microtime(true);

/** Never let an unavailable instrumentation surface emit WordPress database errors. */
...
/** MariaDB restricts INNODB_TRX/INNODB_LOCKS rows to sessions holding PROCESS; record that plainly. */
$process = false;
...
/** MariaDB: an observed LOCK WAIT transaction for the contender is sufficient attribution. */
$mariaState = static function () use ($quiet, $two): ?array {
    $rows = $quiet('SELECT trx_id,trx_state,trx_query FROM information_schema.INNODB_TRX WHERE trx_mysql_thread_id=%d LIMIT 1', array($two));
    foreach ($rows as $row) {
...
$mariaLock = static function () use ($quiet, $two): ?array {
    $rows = $quiet('SELECT l.LOCK_TABLE,l.LOCK_INDEX,l.LOCK_TYPE,l.LOCK_MODE FROM information_schema.INNODB_TRX t'
        . ' JOIN information_schema.INNODB_LOCK_WAITS w ON w.REQUESTING_TRX_ID=t.trx_id'
        . ' JOIN information_schema.INNODB_LOCKS l ON l.LOCK_ID=w.REQUESTED_LOCK_ID'
        . ' WHERE t.trx_mysql_thread_id=%d LIMIT 5', array($two));
    foreach ($rows as $row) return array('mechanism' => 'information_schema_innodb_lock', 'table' => (string) ($row->LOCK_TABLE ?? ''), 'index' => (string) ($row->LOCK_INDEX ?? ''), 'lock_type' => (string) ($row->LOCK_TYPE ?? ''), 'lock_mode' => (string) ($row->LOCK_MODE ?? ''));
    return null;
};

if (!$mysql && !$maria && !$trx) {
    // No instrumentation: allow the contender a bounded, documented settling period.
    usleep(1500000);
} else {
    ...
    while (microtime(true) < $deadline && !is_file($gate . '/w2.result')) {
        ...
        if (!$found && $maria && $evidence === null && $mechanism === 'unavailable' && microtime(true) + 2 > $deadline) $found = $mariaLock();
