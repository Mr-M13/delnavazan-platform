...
foreach(array('wp_get_environment_type','DZN_PHASE_2A2E_GATE_DIR','synthetic_domain','phase-2a2e.invalid','wait_gate','outcome=conflict','proposal_acceptance_events','dzn_phase_2a0_create_ready_teacher_fixture','BookingRequestSubmissionService','recordAdministratorAttestation','issueInitial')as$n)if(strpos($worker.$setup.$verify.$runner.$fixture,$n)===false)throw new RuntimeException('Reproducible Race A-D harness rule missing: '.$n);
