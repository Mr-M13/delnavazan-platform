...
if(strpos($plugin,"DZN_PLATFORM_SCHEMA_VERSION', '11")===false&&strpos($plugin,"DZN_PLATFORM_SCHEMA_VERSION', '12")===false&&strpos($plugin,"DZN_PLATFORM_SCHEMA_VERSION', '13")===false&&strpos($plugin,"DZN_PLATFORM_SCHEMA_VERSION', '14")===false&&strpos($plugin,"DZN_PLATFORM_SCHEMA_VERSION', '15")===false)throw new RuntimeException('Missing compatible provisional acceptance schema marker');foreach(array('011_provisional_acceptance_evidence','proposal_acceptance_events','ENGINE=InnoDB','accepted_pending_conditions','authority_unresolved','dzn_record_booking_request_provisional_acceptance','command_key_digest','command_payload_digest','exact_target','verify_provisional_acceptance_schema')as$n)if(strpos($service.$repo.$idem.$migration.$controller.$plugin,$n)===false)throw new RuntimeException('Missing provisional acceptance invariant: '.$n);
...
foreach(array('wp_get_environment_type','DZN_PHASE_2A2E_GATE_DIR','synthetic_domain','phase-2a2e.invalid','wait_gate','outcome=conflict','proposal_acceptance_events','dzn_phase_2a0_create_ready_teacher_fixture','BookingRequestSubmissionService','recordAdministratorAttestation','issueInitial','c1','c2','d1','d2','x1','x2')as$n)if(strpos($worker.$setup.$verify.$runner.$fixture,$n)===false)throw new RuntimeException('Reproducible Race A-D/X harness rule missing: '.$n);
