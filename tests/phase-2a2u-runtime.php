...
use Delnavazan\Platform\Core\Application\Finance\{FinancePolicyService,LessonFinanceSnapshotService,LessonPayabilityService,TeacherRateService};
...
dzn_u_fix_assert((string)$wpdb->get_var("SELECT status FROM {$p}finance_policies WHERE policy_key='STUDENT_NO_SHOW_COMPENSATION_POLICY' AND policy_version=1")==='superseded','the recorded successor must supersede its predecessor');
...
echo "phase-2a2u-runtime: OK (rate lifecycle, interval resolution, policy guard, snapshot matrix, override)\n";
