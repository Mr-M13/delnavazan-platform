...
foreach(['privacy_erased_at','privacy_erased_by','privacy_erasure_reason','privacy_erased',"state='revoked'",'booking_request_privacy_tombstones','eraseRequestPii','student_id!==null','DELETE FROM','candidate_booking_request_id','FOR UPDATE','signal_digest'] as $needle)if(strpos($migration.$intake.$privacy,$needle)===false)throw new RuntimeException('Missing privacy erasure contract: '.$needle);
