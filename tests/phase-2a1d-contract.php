...
foreach(['idempotency-key','IdempotencyConflictException','reserveSubmissionKey','completeSubmissionKey','WINDOW_SECONDS = 86400','payloadDigest','canonicalize','array_is_list','restartExpiredSubmissionKey'] as $needle)if(strpos($submission.$idempotency.$rest.$intake,$needle)===false)throw new RuntimeException('Missing idempotency contract: '.$needle);
