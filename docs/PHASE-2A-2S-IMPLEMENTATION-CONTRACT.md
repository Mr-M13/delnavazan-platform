...
counter) and is incremented exactly once per acquisition.
...
- The verifier also rejects a **persisted-retry-schedule** violation on a closure that re-arms: an attempt
  ...
  **while an attempt remained and the §9 clamp left a usable window** (`attempt_sequence <
  ...
  rejected as a schedule that must never have been derived.
...
- **Back-off instant, persisted and audited.** `next_available` is the instant derived above, clamped to
  ...
  applied_jitter_bp || ':' || backoff_seconds, wp_salt('dzn_notification'))`. No raw payload and no clock
  reading is stored. A clamp that leaves **no** usable window — the clamped instant is not strictly
...
`available_at`-only re-arm), and the
...
- The retry ceiling is enforced end to end: attempts are acquisition-counted, every re-arm path (the
  ...
  discards the deterministic schedule.
...
- A notification has exactly one outbox row, enforced by unique keys on both sides of that relationship.
