...
| Phase 2A.2-T payment-execution candidate | Schema 28 / migration `028_payment_execution_seam_provider_adapter` / build `phase2a2t-payment-execution-seam-stripe-adapter-20260925.9`, strictly additive on top of the Phase-V candidate tree. Implements the round-9 contract `docs/PHASE-2A-2T-PAYMENT-EXECUTION-SEAM-STRIPE-ADAPTER-CONTRACT.md` (SHA-256 `c6c55119adb4db52ac0583f97317478dd802a2d4ce7498a696ce674f6a951c34`): the provider-neutral execution seam, the provider account/mapping registry, the immutable execution command with its append-only attempt/result rows and its single mutable dispatch claim (sealed descriptor, generation/token fence, reconcile-before-re-issue), the Stripe adapter and its exact-raw-body webhook route — whose `OPTIONS` delivery, which WordPress answers outside normal route dispatch, is intercepted ahead of WordPress's own handler so every registered method is receipted — the durable provider-event intake with its per-event decision claim (the mutable intake row that makes an owed decision recoverable and exactly-once), the bounded ordered R2 consequence, and the authenticated-encryption secret vault. `LIVE_EXECUTION_PROVIDERS` and `PROVISIONABLE_PROVIDERS` are empty, so no outbound call and no provider credential are reachable. **It is a candidate only** — awaiting independent review, not merged, not deployed. **Its contract, migration, runtime, webhook, secret, corruption, failure and nineteen-mode concurrency suites have not been executed: PHP and the disposable WordPress + MariaDB runtime are unavailable in this environment**, and executing them is a mandatory acceptance gate. |
