...
- **O-3 test-count evidence (corrected):** 29 files match `tests/*contract*.php` and all pass. PHP lint (`tests/static.php`) and `sh -n` shell syntax checks are separate syntax checks, not contract tests. The earlier "36 contract tests" figure was a miscount and must not be quoted.
