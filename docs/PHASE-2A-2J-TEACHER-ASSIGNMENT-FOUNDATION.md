...
Teacher archival is rechecked inside a transaction while holding an exclusive Teacher row lock. An applicable Assignment blocks archival. Assignment creation/replacement locks the canonical Enrolment first, then involved Teachers with shared locks in ascending numeric ID order, then concrete Assignment rows. Shared Teacher locks preserve offboarding exclusion while allowing unrelated Enrolments using the same active Teacher to progress independently. Transactions use `READ COMMITTED`; the parent Enrolment row serializes commands while avoiding stale snapshots during an attributed waiter.
...
The disposable runtime suite covers clean Schema 15 → 16 migration, repeat upgrade, capability repair, no-backfill/zero-Assignment reads, historical initial-Teacher continuity, both replacement-evidence routes, end/cancel, closed-Enrolment rejection, every meaningful write-boundary rollback, deliberate duplicate-looking terminal failures, command/Assignment/lifecycle/provenance corruption, database uniqueness, offboarding protection, privacy minimisation, and no Enrolment mutation. The deterministic fourteen-mode two-process matrix covers competing keys, replacement versus end, different/same-successor replacements, both commit orders for initial/replacement versus archival, both commit orders for initial/replacement versus Enrolment closure, unrelated identity roots, and unrelated Enrolments sharing one Teacher. Competing initial Teachers are structurally unavailable because `assignInitial` accepts no Teacher choice and derives the exact retained final-arrangement Teacher.
